...
exports.pageApi = async (page) => {
	if (page.sections.banner.order) {
		page.sections.banner.dataArray = await  getAllForPages(Banner, page.sections.banner.queryParams);
		console.log(page);
	}
	if (page.sections.company.order) {
		page.sections.company.dataArray = await  getAllForPages(Company, page.sections.company.queryParams);
	}
	if (page.sections.services.order) {
		page.sections.services.dataArray = await  getAllForPages(Service, page.sections.services.queryParams);
	}
	if (page.sections.reviews.order) {
		page.sections.reviews.dataArray = await  getAllForPages(Review, page.sections.reviews.queryParams);
	}
	...
	if (page.sections.awards.order) {
		page.sections.awards.dataArray = await  getAllForPages(Award, page.sections.awards.queryParams);
	}
	...
	if (page.sections.workFlow.order) {
		page.sections.workFlow.dataArray = await  getAllForPages(WorkFlow, page.sections.workFlow.queryParams);
	}
	if (page.sections.joinTeam.order) {
		page.sections.joinTeam.dataArray = await  getAllForPages(JoinTeam, page.sections.joinTeam.queryParams);
	}
	...
	if (page.sections.FAQs.order) {
		page.sections.FAQs.dataArray = await  getAllForPages(FAQ, page.sections.FAQs.queryParams);
	}
	...
	if (page.sections.benefits.order) {
		page.sections.benefits.dataArray = await  getAllForPages(Benifit, page.sections.benefits.queryParams);
	}
	...
	if (page.sections.ourWork.order) {
		page.sections.ourWork.dataArray = await  getAllForPages(OurWork, page.sections.ourWork.queryParams);
	}
