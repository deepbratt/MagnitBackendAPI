...
exports.pageApi = async (page) => {
	if (page.sections.banner.order) {
		page.sections.banner.dataArray = await getAll(Banner, page.sections.banner.queryParams);
		console.log(page);
	}
	if (page.sections.company.order) {
		page.sections.company.dataArray = await getAll(Company, page.sections.company.queryParams);
	}
	if (page.sections.services.order) {
		page.sections.services.dataArray = await getAll(Service, page.sections.services.queryParams);
	}
	if (page.sections.reviews.order) {
		page.sections.reviews.dataArray = await getAll(Review, page.sections.reviews.queryParams);
	}
	...
	if (page.sections.awards.order) {
		page.sections.awards.dataArray = await getAll(Award, page.sections.awards.queryParams);
	}
	...
	if (page.sections.workFlow.order) {
		page.sections.workFlow.dataArray = await getAll(WorkFlow, page.sections.workFlow.queryParams);
	}
	if (page.sections.joinTeam.order) {
		page.sections.joinTeam.dataArray = await getAll(JoinTeam, page.sections.joinTeam.queryParams);
	}
	...
	if (page.sections.FAQs.order) {
		page.sections.FAQs.dataArray = await getAll(FAQ, page.sections.FAQs.queryParams);
	}
	...
	if (page.sections.blogs.order) {
		page.sections.blogs.dataArray = await getAll(Blog, page.sections.blogs.queryParams);
	}
	if (page.sections.benefits.order) {
		page.sections.benefits.dataArray = await getAll(Benifit, page.sections.benefits.queryParams);
	}
	...
	if (page.sections.ourWork.order) {
		page.sections.ourWork.dataArray = await getAll(OurWork, page.sections.ourWork.queryParams);
	}
